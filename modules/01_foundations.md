...
> 这一章不是为了变成数学家，主要为了帮助理解 AI 的底层逻辑：知道模型为什么能学习、为什么会不稳定等等，了解底层就会祛魅，大模型学起来也不会像看起来的那样难啃
>
> 第一次阅读这章时，我推荐优先看每个知识点的 `为什么重要 / 核心问题`，先抓住核心重点，内容很多不必一上来就死抠每个知识点，特别数学公式。
>
> 返回 [知识库索引](/SYLLABUS)
...
- **如果要面试，很急 **：那就先抓 `loss / optimizer / norm / 梯度 / 过拟合` 这些高频词，热点内容，知道它们分别在解决什么问题
- **如果你是非科班**：不用按顺序硬啃，先看每个知识点的 `为什么重要`，遇到项目或面试再回来补细节会更轻松
- **如果你准备连续往后看 Transformer / RAG / Agent**：这一章的目标就是建立后面让你看得懂、学得下去的基础内容
- **建议第一遍优先看**：`Cross-Entropy`、`AdamW`、`LayerNorm`、`Warmup`、`梯度消失/爆炸`、`过拟合`这些内容

---

## 1.1 数学基础：目标就是有点感觉

> 这里的数学不是为了考试，而是为了看懂模型里的几个核心动作：表示、相似度、概率、优化和稳定性。
>
> **第一次读到这里，只要先回答一个问题就够了：这些数学概念，分别在模型里“干什么、管什么”。**

- **线性代数**
  - *先理解什么*：向量/矩阵如何表示信息，模型怎样做“比较、投影、压缩”
  - *关键词*：矩阵乘法、转置、逆矩阵、特征值分解、SVD
  - *为什么重要*：Attention 本质是矩阵运算；SVD 是 LoRA 低秩分解的理论基础

- **概率统计**
  - *先理解什么*：模型为什么输出“概率”，以及“不确定性”该怎么表达
...
- **评估指标**
  - *需要知道*：不能只看一个指标就觉得模型是ok的，专业的方式都是需综合衡量
  - *常见指标*：Accuracy / Precision / Recall / F1
  - *核心问题*：为什么不能只看正确率（Accuracy)--经典场景解释，医院的癌症检测，100个人中99个人正常，一个人癌症，大模型都给正常，难道就能说检测正确率是99%吗？
...
> 深度学习就是“传统机器学习”走向“神经网络世界”的桥梁，后面涉及的 Transformer、RAG、Agent都是从这一部分衍生而来。
>
...
- **CNN 核心概念**
  ...
  - *为什么重要*：理解残差连接对后续理解 Transformer 很重要
...
- **归一化方法** ⭐ 🔥
  - *需要知道*：归一化就是为了防止数值太大爆炸或太小消失，让模型训练更稳、更快。
