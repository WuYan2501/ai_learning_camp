...
> 这一章不是为了把你变成数学家，而是先建立理解 AI 的底层直觉：知道模型为什么能学、为什么会不稳定、为什么很多经典概念到了大模型时代依然重要。
>
> 第一次阅读时，优先看每个知识点的 `为什么重要 / 核心问题`，先建立感觉，不必一上来就抠公式。
>
> 返回 [知识库索引](/SYLLABUS)
...
- **如果你刚开始接触 AI**：先抓 `loss / optimizer / norm / 梯度 / 过拟合` 这些高频词，知道它们分别在解决什么问题
- **如果你是非科班**：不用按顺序硬啃，先看每个知识点的 `为什么重要`，遇到项目或面试再回来补细节会更轻松
- **如果你准备继续往后看 Transformer / RAG / Agent**：这一章的目标不是“全会推导”，而是建立后面看得懂、学得下去的底层感觉
- **建议第一遍优先看**：`Cross-Entropy`、`AdamW`、`LayerNorm`、`Warmup`、`梯度消失/爆炸`、`过拟合`
...
>
> **第一次读到这里，只要先回答一个问题就够了：这些数学概念，分别在模型里“管什么”。**
