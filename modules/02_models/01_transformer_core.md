...
- **RoPE（旋转位置编码）**：他就是这部分内容的主角,核心原因是市面上开源的一些大模型`Qwen`、`Kimi 2.5`、`LLaMA`、`DeepSeek`都用的是它。

如果让你来做给词加位置信息，你是否会很自然的用List列表的形式来记录？它天然带有位置信息，但是这个给大模型理解，就存在两个很大的弊病，
当遇到**超长的词句不知道如何理解**，例如我们给第 1 个词标 1，第 2 个词标 2……这种长度永远存在长度局限，当模型训练时max_token限制为 4096，那真实场景遇到4097长度的内容怎么办？它从没见过这个编号，那不是直接就懵了
